...
// Taken from linux_src/samples/bpf/bpf_insn.h
...
#define BPF_LD_IMM64_RAW(DST, SRC, IMM)                                                                                                              \
	((struct bpf_insn){.code = BPF_LD | BPF_DW | BPF_IMM, .dst_reg = DST, .src_reg = SRC, .off = 0, .imm = (__u32) (IMM)}),                          \
...
#define BPF_MOV64_RAW(TYPE, DST, SRC, IMM)                                                                                                           \
	((struct bpf_insn){.code = BPF_ALU64 | BPF_MOV | BPF_SRC(TYPE), .dst_reg = DST, .src_reg = SRC, .off = 0, .imm = IMM})

#define BPF_MOV32_RAW(TYPE, DST, SRC, IMM)                                                                                                           \
	((struct bpf_insn){.code = BPF_ALU | BPF_MOV | BPF_SRC(TYPE), .dst_reg = DST, .src_reg = SRC, .off = 0, .imm = IMM})
...
#define BPF_LDX_MEM(SIZE, DST, SRC, OFF)                                                                                                             \
	((struct bpf_insn){.code = BPF_LDX | BPF_SIZE(SIZE) | BPF_MEM, .dst_reg = DST, .src_reg = SRC, .off = OFF, .imm = 0})
...
#define BPF_ATOMIC_OP(SIZE, OP, DST, SRC, OFF)                                                                                                       \
	((struct bpf_insn){.code = BPF_STX | BPF_SIZE(SIZE) | BPF_ATOMIC, .dst_reg = DST, .src_reg = SRC, .off = OFF, .imm = OP})
...
#define BPF_ST_MEM(SIZE, DST, OFF, IMM)                                                                                                              \
	((struct bpf_insn){.code = BPF_ST | BPF_SIZE(SIZE) | BPF_MEM, .dst_reg = DST, .src_reg = 0, .off = OFF, .imm = IMM})
...
#define BPF_JMP32_REG(OP, DST, SRC, OFF)                                                                                                             \
	((struct bpf_insn){.code = BPF_JMP32 | BPF_OP(OP) | BPF_X, .dst_reg = DST, .src_reg = SRC, .off = OFF, .imm = 0})

/* Like BPF_JMP_IMM, but with 32-bit wide operands for comparison. */

#define BPF_JMP32_IMM(OP, DST, IMM, OFF)                                                                                                             \
	((struct bpf_insn){.code = BPF_JMP32 | BPF_OP(OP) | BPF_K, .dst_reg = DST, .src_reg = 0, .off = OFF, .imm = IMM})
...
#define BPF_CAST_CALL(x) ((u64(*)(u64, u64, u64, u64, u64))(x))

#define BPF_EMIT_CALL(FUNC) ((struct bpf_insn){.code = BPF_JMP | BPF_CALL, .dst_reg = 0, .src_reg = 0, .off = 0, .imm = ((FUNC) -__bpf_call_base)})
...
#define BPF_ST_NOSPEC() ((struct bpf_insn){.code = BPF_ST | BPF_NOSPEC, .dst_reg = 0, .src_reg = 0, .off = 0, .imm = 0})
